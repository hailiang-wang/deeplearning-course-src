...
model = torch.nn.Sequential(OrderedDict([
    ('hidden_layer1', nn.Linear(1, 64)),
    ('hidden_layer1_activation', nn.Tanh()),
    ('output', nn.Linear(64, 1)),
]))
# summary(model)

print("weights of hidden layer", model.hidden_layer1.weight)
print("bias of hidden layer", model.hidden_layer1.bias)

for name, params in model.named_parameters():
    print(name, params)

# https://zhuanlan.zhihu.com/p/704991790
loss_fn = nn.MSELoss()
...
for epoch in range(1, epochs + 1):
    ...
    # print("t_p shape", t_p.squeeze(dim=-1).shape)
    # print("train_data_y shape", train_data_y.shape)
    # sys.exit()

    loss = loss_fn(t_p.squeeze(dim=-1), train_data_y)
    ...
    if epoch % 100 == 0:
        with torch.no_grad():
            ...
            valid_loss = loss_fn(model(validate_data_x).squeeze(dim=-1), validate_data_y)
