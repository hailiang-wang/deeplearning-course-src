#############################################
# 定义一个参数规模更大的神经网络
# 将验证集分成独立的，输入模型，减少 GPU 显存占用
# PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True python3 ai399_9_2.py
#############################################
import os
import sys
...
cifar10 = datasets.CIFAR10(data_path, train=True, download=False, transform=transforms.Compose([
    ...
    # https://docs.pytorch.org/vision/main/generated/torchvision.transforms.RandomHorizontalFlip.html
...
# resnet_weights = PRETRAINED_MODELS.ResNet50_Weights.DEFAULT
# resnet_model = PRETRAINED_MODELS.resnet50(resnet_weights)
# resnet_weights = PRETRAINED_MODELS.ResNet152_Weights.DEFAULT
# resnet_model = PRETRAINED_MODELS.resnet152(resnet_weights)


def add_custom_layers(model, out_number):
    '''
    使用迁移学习，加载模型 model, 在 model 之后构建新的隐含层和全连接输出，实现新的分类模型
    * 对加载进来的模型，不调整参数
    '''
    ...
    print("[INFO] add_custom_layers num_features %s" % num_features)
...
if __name__ == "__main__":
    ...
    for epoch in range(n_epoches):
        ...
        with torch.no_grad():
            ...
            for x in validate_inputs:
                o = model(x.unsqueeze(dim=0))
                if validate_output is None:
                    validate_output = o
                else:
                    validate_output = torch.cat((validate_output, o), dim=0)
            ...
            if validate_accuracy.item() >= 0.95:
                print("Early stop due to accuracy is above 0.95")
                break
